...
{
	int error;
	int ret = driver_->WriteByte(id, TORQUE_ENABLE, flag, &error);
    usleep(CONTROL_PERIOD*100);
    
	if( ret != COMM_RXSUCCESS ) {
		ROS_ERROR("setTorqueEnable Error. id=%d", id);
		...
		*/
		...
		if (result == COMM_RXSUCCESS) {			
			if (info.joint_type == "prismatic") {	
				...
				double val_rad = ((double) val3 / (double) info.cpr) * 2.0 * M_PI;				
				joint_states_.position[i]  = rad2m( info.max_rad, info.min_rad, info.max_m, info.min_m, val_rad );
								//ROS_INFO("val_rad = %5.2f, pos in m = %5.2f", val_rad, joint_states_.position[i]);								
				}
			else {
				joint_states_.position[i] = ((double) val3 / (double) info.cpr) * 2.0 * M_PI;
				}
			}
...
bool spin()
{
    ...
    {		
	  ...
	  while(ros::ok() && node_handle_.ok()) {
          ...
          if (bJointCommandReceived_) SetJointCommands();
