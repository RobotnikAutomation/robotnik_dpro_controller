...
Dynamixel DXL("/dev/ttyUSB_DPRO1");
...
int main()
{
    ...
    DXL.WriteByte(id_, TORQUE_ENABLE, 0, &error);
    ...
    while (1) {
    	switch(_getch()) {
	case 0x1b:
		DXL.WriteByte(id_, TORQUE_ENABLE, 0, 0);
		usleep(CONTROL_PERIOD*100);
		DXL.WriteByte(id_, OPERATING_MODE, 3, 0);
		break;
	case 0x30:
		DXL.WriteByte(id_, OPERATING_MODE, 0, 0);
        	usleep(CONTROL_PERIOD*100);
		DXL.WriteByte(id_, TORQUE_ENABLE, 1, 0);
        	usleep(CONTROL_PERIOD*100);
		// Goal Torque (604) size is 2 bytes (a word) 
		// WriteWord is the appropriate function
		DXL.WriteWord(id_, GOAL_TORQUE, GoalTorque, &error);
		...
		result = DXL.ReadWord(id_, PRESENT_CURRENT, &val1, &error);
		if (result == COMM_RXSUCCESS)
			printf( "%d\n", val1);
		break;
	case 0x31:
		DXL.WriteByte(id_, OPERATING_MODE, 1, 0);
        	usleep(CONTROL_PERIOD*100);
		DXL.WriteByte(id_, TORQUE_ENABLE, 1, 0);
        	usleep(CONTROL_PERIOD*100);
		// Goal Velocity (600) size is 4 bytes (a double-word)
		// WriteDWrod is the appropriate function
		DXL.WriteDWord(id_, GOAL_VELOCITY, GoalVel, &error);
		usleep(CONTROL_PERIOD*200);
		// after an arbitrary time (i.e. 2 secs in this case)
		// output onscreen
		// Present Velocity (615) size is 4 bytes (a double-word)
		// ReadDWord is the appropriate function
		result = DXL.ReadDWord(id_, PRESENT_VELOCITY, (long*) &val2, &error);
		if (result == COMM_RXSUCCESS)
			printf( "%d\n", val2);
		break;
	default:		
		...
		// WriteDWrod is the appropriate function
		DXL.WriteDWord(id_, GOAL_POSITION, GoalPos, &error);
		// after an arbitrary time (i.e. 2 secs in this case)
		// output onscreen
		// Present Position (611) size is 4 bytes (a double-word)
		// ReadDWord is the appropriate function
		usleep(CONTROL_PERIOD*200);
		result = DXL.ReadDWord(id_, PRESENT_POSITION, (long*) &val3, &error);
		if (result == COMM_RXSUCCESS)
			printf( "%d\n", val3);
		break; }
