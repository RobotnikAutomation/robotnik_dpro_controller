...
int main()
{
    ...
    DXL.WriteByte(11, P_TORQUE_ENABLE, 0, 0);
    usleep(CONTROL_PERIOD*100);    
	DXL.WriteByte(11, OPERATING_MODE, 1, 0);
	usleep(CONTROL_PERIOD*100);
    DXL.WriteByte(11, P_TORQUE_ENABLE, 1, 0);
    usleep(CONTROL_PERIOD*100);         
    
    DXL.WriteByte(12, P_TORQUE_ENABLE, 0, 0);
    usleep(CONTROL_PERIOD*100);    
	DXL.WriteByte(12, OPERATING_MODE, 1, 0);
	usleep(CONTROL_PERIOD*100);
    DXL.WriteByte(12, P_TORQUE_ENABLE, 1, 0);
    usleep(CONTROL_PERIOD*100);         
    ...
    while(1)
    {
        ...
        do
        {
            ...
            for(i = 0; i < NUM_ACTUATOR; i++)
            {
                GoalPos = (int)((sin(theta+phase[i])) * (double)AmpPos) / 4;
            ...
            result = DXL.SyncWrite(GOAL_VELOCITY, 4, param, NUM_ACTUATOR*(1+4));
