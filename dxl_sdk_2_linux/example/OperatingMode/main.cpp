...
int main(int argc, char **argv)
{
    ...
    int GoalPos = 1000; // for L42-series ???
    //int GoalVel = -3000;
    int GoalVel = 8000;
    // int GoalTorque = 120;
    int GoalTorque = 5;
	...
	case 0x30:
		...
		DXL.WriteWord(id_, GOAL_TORQUE, GoalTorque, 0);
		break;
	case 0x31:
		...
		goalvel32 = (int32_t) GoalVel;
		DXL.WriteDWord(id_, GOAL_VELOCITY, goalvel32, 0);
		break;
	default:		
		DXL.WriteByte(id_, OPERATING_MODE, 3, 0);
		usleep(CONTROL_PERIOD*100);
		...
		// WriteDWrod is the appropriate function
		DXL.WriteDWord(id_, GOAL_POSITION, GoalPos, 0);
		break; 
		}
	...
	if (result == COMM_RXSUCCESS) printf( "vel = %d \n", val2);
	...
	if (result == COMM_RXSUCCESS) printf( "pos = %d\n", val3);
	}
